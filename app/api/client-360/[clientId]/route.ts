...
export async function GET(_request:Request,{params}:{params:Promise<{clientId:string}>}){
  ...
  const timeline=[
    ...
    ...(downloadsResult.data||[]).map(row=>({id:`download:${row.id}`,type:'download',title:`Orion ${row.version||'release'} delivery requested`,detail:row.platform||'Product download request',date:row.downloaded_at,tone:'cyan'})),
