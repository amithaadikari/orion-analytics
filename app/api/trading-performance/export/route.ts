import { z } from 'zod';
import { getPortalSession } from '@/lib/portal-session';
import { createSupabaseAdminClient } from '@/lib/supabase/server';
import { jsonError } from '@/lib/security';
import { isTradingAnalyticsRange } from '@/lib/trading-analytics';
import {
  buildTradingPerformanceCsv,
  canExportTradingPerformance,
  tradingPerformanceCsvFilename,
} from '@/lib/trading-performance';
import {
  loadClientTradingPerformance,
  publicTradingPerformanceError,
} from '@/lib/trading-performance-server';

export const dynamic = 'force-dynamic';

const querySchema = z.object({
  connectionId: z.string().uuid(),
  range: z.string().refine(isTradingAnalyticsRange),
}).strict();

export async function GET(request: Request) {
  const session = await getPortalSession();
  if (!session.user) return jsonError('Authentication required', 401);
  if (session.mfaRequired) return jsonError('Authenticator verification required', 403);
  if (!session.client) return jsonError('A linked Orion client account is required', 403);

  const url = new URL(request.url);
  if ([...url.searchParams.keys()].some((key) => !['connectionId', 'range'].includes(key))) {
    return jsonError('Invalid performance export request', 400);
  }
  const parsed = querySchema.safeParse(Object.fromEntries(url.searchParams.entries()));
  if (!parsed.success) return jsonError('Invalid performance export request', 400);

  try {
    const snapshot = await loadClientTradingPerformance(
      createSupabaseAdminClient(),
      session.client.id,
      parsed.data,
    );
    if (snapshot.availability !== 'ready' || !snapshot.performance
      || !canExportTradingPerformance(snapshot.access)) {
      return jsonError('Premium or Lifetime is required for performance CSV exports.', 403);
    }
    return new Response(buildTradingPerformanceCsv(snapshot), {
      headers: {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="${tradingPerformanceCsvFilename(snapshot)}"`,
        'Cache-Control': 'private, no-store',
        'X-Content-Type-Options': 'nosniff',
      },
    });
  } catch (error) {
    const mapped = publicTradingPerformanceError(error);
    return jsonError(mapped.message, mapped.status);
  }
}
