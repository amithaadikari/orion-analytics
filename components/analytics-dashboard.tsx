...
export default function Dashboard({ admin }: DashboardProps) {
  ...
  const load = useCallback(async () => { setLoading(true); const params = new URLSearchParams({ range }); if (range === 'custom' && customStart) params.set('start', new Date(`${customStart}T00:00:00Z`).toISOString()); if (range === 'custom' && customEnd) params.set('end', new Date(`${customEnd}T23:59:59.999Z`).toISOString()); if (country !== 'all') params.set('country', country); if (campaign !== 'all') params.set('campaign', campaign); if (device !== 'all') params.set('device', device); if (eventFilter !== 'all') params.set('event', eventFilter); const response = await fetch(`/api/dashboard?${params}`, { cache: 'no-store' }); if (response.ok) setSnapshot(await response.json()); setLoading(false); }, [range, customStart, customEnd, country, campaign, device, eventFilter]);
  useEffect(() => { load(); }, [load]);
