...
function AdminPortalProfile({client,profile}:{client:Data['client'];profile:AdminProfile}){
 ...
 return <section className={styles.portalProfile} aria-labelledby="admin-portal-profile-title">
  <div className={styles.profileIdentity}><ClientAvatar avatarKey={profile.avatarKey} size="large"/><div><small>Client-selected identity</small><h4 id="admin-portal-profile-title">{displayName}</h4><p>{client.full_name}{client.country?` · ${client.country}`:''}</p></div><span data-complete={completed}>{completed?'Profile updated':profile.linked?'Not completed':'Portal not linked'}</span></div>
  {!profile.available&&<p className={styles.profileWarning} role="status">Portal preferences are temporarily unavailable. Contact details below are still current.</p>}
  <div className={styles.profileFacts}><span><small>Nickname</small><strong>{profile.nickname||'Not added'}</strong></span><span><small>Telegram</small><strong>{profile.telegramUsername?`@${profile.telegramUsername}`:'Not added'}</strong></span><span><small>Phone</small><strong>{profile.phoneNumber||'Not added'}</strong></span><span><small>Last profile update</small><strong>{profile.updatedAt?new Date(profile.updatedAt).toLocaleString():'No client update yet'}</strong></span></div>
  <div className={styles.profileBio}><small>Trading bio</small><p>{profile.bio||'The client has not added a trading bio.'}</p></div>
  <div className={styles.profilePreferences}><ProfileValues label="Brokers used" values={profile.brokers}/><ProfileValues label="Most-traded markets" values={profile.tradingPairs}/></div>
  <p className={styles.profilePrivacy}>Client-provided portal preferences. These details do not change plan, license, payment, or account status.</p>
 </section>
}

function ProfileValues({label,values}:{label:string;values:string[]}){return <div><small>{label}</small><span>{values.length?values.map(value=><i key={value}>{value}</i>):<b>None selected</b>}</span></div>}
